...
type Person = {
  id?: string;
  name: string;           // 名前（漢字）
  nameKana?: string;      // なまえ（ひらがな）
  yobikata?: string;      // 呼び方（例：大和さん、やまとくん、やまと）
  company?: string;
  phone?: string;
  email?: string;
...
export default function PersonEditPage() {
  ...
  useEffect(() => {
    return onAuthStateChanged(auth, async (u) => {
      ...
      setUid(u.uid);
      try {
        const ref = doc(db, "users", u.uid, "people", params.id);
        const snap = await getDoc(ref);
        if (snap.exists()) setP({ id: snap.id, ...(snap.data() as Person) });
      } finally {
        setLoading(false);
      }
    });
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const ref = doc(db, "users", uid, "people", params.id);
      await setDoc(
        ref,
        {
          ...p,
  ...
  const remove = async () => {
    ...
    try {
      await deleteDoc(doc(db, "users", uid, "people", params.id));
      alert("削除しました。");
      router.push("/people");
    } catch (e) {
    ...
    <Container>
      <Card className="space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-base font-bold">会った人を編集</h1>
